...
// ar## : #-# 
// Since I use Arduino mega, I facilitate A1~A15 from board and 0~9 channel from multiplexer(CD74HC4067)
//using readMux function, read channel of multiplexer and store in the variable 
...
// Set up for reading Mux

  int readMux(int channel)  { 

  //multiplexer control pins (s0, s1, s2, s3) are stored in a controlPin[] array
  int controlPin[] = {s0, s1, s2, s3}; 

  //The control signals corresponding to each multiplexer channel are stored. 
  //Each channel is represented by 4 bits.
  int muxChannel[16][4]={
  {0,0,0,0}, //channel 0 
  {1,0,0,0}, //channel 1 
  {0,1,0,0}, //channel 2 
  ...
  //loop through the 4 sig, setting the appropriate control signals for the given channel using the digitalWrite function.
  //If the channel is 3, muxChannel[3] is {1, 1, 0, 0}, which sets the s0, s1, s2, s3 pins to HIGH, HIGH, LOW, LOW, respectively
  for(int i = 0; i < 4; i ++){ 
